...
function checkExecutableSkill_CreatorFlame(obj)
{
	...
	{
		obj.sq_IntVectClear();
		obj.sq_IntVectPush(SUB_STATE_CREATORFLAME_0); // substate세팅
...
function checkCommandEnable_CreatorFlame(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_CREATORFLAME); // 결투장에서는 특정스킬만 캔슬이 가능합니다. 작업자:정진수 [2012.04.20] obj.sq_IsCommandEnable(SKILL_BROKENARROW);
	}
