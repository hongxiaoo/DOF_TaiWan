...
// state를 세??고 처음으로 들어오게 됩니다. 각종 리소스를 세??니다. 
...
function onAfterSetState_Attack(obj, state, datas, isResetTimer)
{
	...
	// 수인체 스킬을 배웠다면, ?타 에니메이션이 바뀜
	// 마법구도 발사되지 않음.



	if (state == STATE_ATTACK && !skill)
	{
		// ?타 이??를 붙인다.
		// 걸려있는 버? 스킬 인덱스 및 서브 스테이?에 따라서 보여지는 이??가 다름 (속성에따라서 붙는 이??가 다름)
		local element = obj.getThrowElement();
		local attackIndex = obj.getAttackIndex();
		
		// 속성발동 appendage에 걸려있는지 체크?.
		// 속성발동에 걸려있지 않다면 무속성 ?타가 발사됨
...
function createMiniMagicCircle(obj, xPos, zPos, shotDirection, bonusDamage) // direction 0:정면  1:대각선  2:?단)
{	
	if(!obj)
		return;
	if (obj.sq_IsMyControlObject()) {
		...
		{	// 정면
			...
			// 연속발사는 생김새는 같지만, 다른 오브젝?가 발사됨 (컨버전의 영?을 받기때문)
...
function onKeyFrameFlag_Attack(obj, flagIndex)
{
	...
	// 공격?때마다 마법 구체를 생성?다.
	// 걸려있는 버?에 따라서 생성 되어져야 ? ?시브 오브젝?가 다름(속성)
	// 
	local xPos = 0, zPos = 0;
	...
	{
		// 마법구체를 생성?다.
		createMiniMagicCircle(obj, xPos, zPos, 0, 0); // direction 0:정면  1:대각선  2:?단
	}
